...
class TestEvaluateEnglishTextWithModel(unittest.TestCase):
    ...
    def test_with_model_configured(self, mock_run, mock_base_cmd):
        ...
        # Verify subprocess.run received cmd from _base_claude_cmd + prompt
        called_cmd = mock_run.call_args[0][0]
        self.assertEqual(called_cmd[:4], ["/usr/bin/claude", "-p", "--model", "sonnet"])
        # Last element is the prompt
        self.assertIn("Add button", called_cmd[-1])
...
class TestEvaluateEnglishTextBadJson(unittest.TestCase):
    ...
    def test_non_json_response_returns_none(self, mock_run, mock_base_cmd):
        mock_run.return_value = MagicMock(stdout="This is not valid JSON at all", returncode=0)
